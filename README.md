...
**0.1.0-lab.2** (the "-lab" series is Lablooms' pre-release track). Continuous segmented capture + rolling buffer + concat-to-save,
...
with a thumbnail preview. A first-run wizard runs on a fresh install, a second launch
...
- **Linux Wayland sessions only**: GStreamer with the PipeWire plugin
  ...
  Wayland caveat); a global hotkey still isn't possible there, so use
...
- **Logs**: the log viewer, with search and level filters over the rotating log file's
...
0.5× to 2× speed, plus a built-in trim editor (mark start/end from the playhead or type
...
(start/duration/frame rate/width) and **Compress…**
...
Launching a second copy while one is already running shows an "already running"
...
dismissible banner on the Home tab if one's found. It's notify-only: it never downloads
or installs anything for you. The Settings row also shows when the last check happened
